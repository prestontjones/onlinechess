...
public class ChessServer {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 3. Start the Server (in a new thread)
            server.start();
        ...
        try {
            // 4. Register network classes
            info("Registering network packets...");
            NetworkRegistry.register(server.getKryo());
            info("Packet registration complete.");
        } catch (Exception e) {
            error("Failed to register network classes.", e);
            server.stop();
            System.exit(1);
        }
        
        try {
            // 5. Add the network listener
            info("Adding server listener...");
            ServerNetworkListener listener = new ServerNetworkListener(server);
            server.addListener(listener);
            info("Listener successfully added.");
        } catch (Exception e) {
            error("Failed to add server listener.", e);
            server.stop();
            System.exit(1);
        }
